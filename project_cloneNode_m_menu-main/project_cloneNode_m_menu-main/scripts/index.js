...
// m_nav_btn.addEventListener('click',()=>{
    ...
    //위 두상황을 컴퓨터가 인식하는 0, 1 로 구분해서 생각한다
    //메뉴가 보인다 == 1, 메뉴가 안보인다 == 0
    //사용자가 처음 적속 시 안보이는 걸로 시작하는지 보이는 걸로 시작하는지에 따라
    //해당 값을 변수의 초기값으로 저장한다.
    // let m_showHide_Boolean = 0; //안보이는 걸로 시작한 초기값
    //사용자의 행동에 따라 변경된 초기값 예측을 위해 조건식으로 작업
    // m_showHide_Boolean==0 ? m_navfunc('#fff', '0%', 1) : m_navfunc()
    // 조건 ? 조건이 참일때 실행 : 조건이 거짓일때 실행
    //m_showHide_Boolean의 값이 0이 맞으면, 아니면에 따라 각각 다른결과가 실행
    //맞다<->아니다가 반복해서 돌아야하므로 실행되는 함수내에
